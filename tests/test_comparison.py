...
from chansu.reasoning.design_reasoning import DesignReasoning, ReasoningCheck
...
def test_readme_includes_each_model_and_declines():
    ...
    reasonings = {
        "Claude": DesignReasoning(
            model_name="claude", available=True, narrative="A synthesis paragraph.",
            rationales={("membrane_toxicity", "soft_drug"): "Rationale text."},
            checks=[ReasoningCheck(label="synthesis", passed=True, stop_reason="end_turn")],
        ),
    ...
    assert "[reasoning · claude]" in md                                    # section names its backend
    assert "- **membrane_toxicity / soft_drug:** Rationale text." in md    # rationale bullet rendered
    assert "_Checks: 1/1 cleared._" in md                                  # checks summary rendered
